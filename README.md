...
That boots the simulator and launches a standalone Release build — **no Metro or dev server required**. See [TESTING.md](TESTING.md) for a walkthrough of what to try. Use `npm run sim:build` to rebuild it after changing code, or `npx expo run:ios` for the usual live-reload dev loop.
...
| `scripts/sim.sh` | Boots a simulator and runs the standalone build. |
