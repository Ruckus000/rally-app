...
**What is still local, and is the thing to know.** Nothing writes `week_rollups`. Your history, your running totals and week rollover are computed on the device and persisted only to AsyncStorage — see the note at `src/sync/mappers.ts`, which counts the rows it already has rather than reading a rollup nothing produces.
...
Two halves of that are still missing, and both are named rather than implied. **Android has no way back**: `expo-apple-authentication` is iOS-only, and Google is the outstanding provider. And **recovery returns your data, not your history** — identity, circle, invite code, the current week, notes and reactions all come back, but the totals and the year grid do not, because they are the rollups nothing writes yet.
