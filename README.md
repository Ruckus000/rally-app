...
**What a closed week leaves behind.** `week_rollups` is written when you confirm a rollover and read back on a device that has none — so history, the year grid and every total on Me survive a reinstall. The row carries five numbers and a date; `n`, the label and the summary line are rebuilt from those, and the totals are derived by `aggregatesFrom` rather than stored, so there is one definition of what "all-time points" means. `did` is deliberately empty on a restored week: the titles are already on the server in `tasks`, and duplicating them into the rollup to save a read nobody currently makes is the wrong trade.

Every account still *starts* as an anonymous sign-in, and until recently that meant deleting the app cost you the account permanently. **On iOS it no longer does.** Me offers "Secure this account", which attaches an Apple identity through `linkIdentity` and a native id token — the account keeps its uuid, so nothing it owns has to move — and "Continue with Apple" on the Welcome screen signs it back in on a device that has never seen it. `src/lib/appleAuth.ts` is the native edge; `src/sync/session.ts` holds the policy.

One half of that is still missing, and it is named rather than implied: **Android has no way back**, because `expo-apple-authentication` is iOS-only and Google is the outstanding provider.
