...
- **On iOS an account can now be got back; on Android it still cannot.** Me offers "Secure this account" on a live iOS account, which attaches an Apple identity and keeps the same user id. "Continue with Apple" on the Welcome screen then signs that account back in on a fresh install. `expo-apple-authentication` is iOS-only, so an Android account has no way back until Google sign-in lands, and there both buttons stay inert under the coming-soon line.
- **Recovery returns your data, not your history — yet.** Signing back in restores identity, name, circle, invite code, the current week, notes, reactions and notifications. It does **not** restore history, streaks or the Me screen's totals, because nothing writes `week_rollups`; that is PR 2 of this wave. Until it lands, a recovered account is genuinely back but reads as a zeroed Me screen.
