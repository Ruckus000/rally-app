...
- **`screen-task-media` has been run live against the deployed project, once.** A generated 900×600 JPEG went `pending` → `ready` in 1.8s with a real `GEMINI_API_KEY` call, and the three non-model branches answered correctly: `waiting` while the object existed but the row had not landed, the settled state on a repeat call with no second model call, and `refused` for a media id nothing claims. The test row, object, task and anonymous user were deleted afterwards. What that does **not** cover is a photo the model actually objects to — the refusal path has still only ever been driven against a stub.
- **`screen-image` has never been called live.** Its wire shape was exercised against a stub `LLM_BASE_URL` (see `supabase/functions/README.md`) and documented-verified against Gemini's published response shape. The first real avatar upload against the deployed function is genuinely untested.
