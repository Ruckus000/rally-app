...
- **The two-device test has now been run, on 2026-08-23, and it passed.** Two live accounts on two simulators — an iPhone 16 Pro and an iPhone 17 Pro — against the deployed project, not a local stack. Seven legs, all green: anonymous sign-in on both devices with distinct user ids; the first `pull_world` bringing down the four Oz bots with their real uuids; a circle created and a server-issued invite code (`tw-…`) handed out; the second account joining by that code; the membership propagating *back* to the first device with no manual refresh; a goal closed on one device appearing in the other's feed inside a minute, labelled FRIENDS and sorted above the FOLLOW bots; and a cheer sent from the second device landing in the first's notifications under *Your circle*. Audience routing was correct in both directions — the thing RLS is for. Nothing needed the paid Apple programme: "Get started" flips the account to live and signs in anonymously, so the whole loop is reachable on a free build.

  What it did **not** cover, and what the rest of this list still means: physical devices (it was two simulators), photos, reporting and blocking, avatar upload, Android, and the Apple recovery path.

  One trap, if you automate this. Driving the flow with a script, the name came out as its first letter — "Ana" became "A" — and that is **not** a bug in the app. It is a race between the tap that focuses the field and keystrokes arriving a millisecond later, which no person can reproduce: typed one character at a time it is fine, typed as a burst into a field that has been focused for even a second it is fine, and typed into a field that already has content it is fine. Only tap-then-immediately-type loses characters. `patch` uses the functional form of `setState`, `NAME_MAX` is 80, and nothing on the input path slices. Put a pause after focusing a field before you decide the app dropped your text.
...
- **Nothing verifies either image screener is any good.** What's tested is the plumbing and the fail-closed verdict shape (`imageVerdict.mjs`, pinned in `src/lib/__tests__/imageVerdict.test.ts`) — not whether the model's judgement on an actual photo is right. That needs a corpus of real images and a human looking at the results, which no pass has built — and which is the one gap here that cannot be closed by testing harder, only by looking at real photographs. The goal-photo prompt (`GOAL_IMAGE_SCREENING`) is a harder case than the avatar one and less exercised: most goal photos have no face in them, so the "answer no to everything else" list is doing more work than usual.
- **Every branch of `screen-task-media` has been driven through the real Deno runtime, with only the model stubbed.** Point `LLM_BASE_URL` at a stub (see `supabase/functions/README.md`) and all six behave: a clean answer publishes and keeps the object; harmful, a `SAFETY` refusal, an unparseable body, a 503, and being over the daily cap each answer `refused` and leave **no row and no object**. The cap refuses before it makes any model call at all. That exercises the real delete-and-mark code against a real Postgres and real storage — what is faked is the judgement, not the consequence.
- **The unscreenable-MIME branch is unreachable from a client.** The bucket's own `allowed_mime_types` refuses anything outside jpeg/png/webp at upload, so `SCREENABLE` inside the function is defence against a misconfigured bucket rather than a path anyone can take. Worth knowing before someone deletes it as dead code.
- **`screen-task-media` has also been run live against the deployed project.** A generated JPEG went `pending` → `ready` in 1.8s on a real `GEMINI_API_KEY` call, `waiting` came back while the object existed but the row had not landed, a repeat call returned the settled state with no second model call, and an unclaimed media id answered `refused`. Test data deleted afterwards.
- **`screen-image` has been called live, once.** Upload → `set_avatar` → `pending` → **`ready` in 2.3s** on a real model call, and a second call returned the settled state without spending another. Its refusal branches share `imageVerdict.mjs` with `screen-task-media`, so they are covered by the stub run above; what has never run is `screen-image`'s own delete-and-mark against production, since that needs an image the model objects to.
...
- **A friend's photo reaches your feed, and the gate is what decides whether it may.** `pull_world` carries an eleventh key with no predicate of its own — it is SECURITY INVOKER, so `task_media_select` answers screening state, blocks and audience. The gate landing *before* the read path is why that works: there was never an ungated fetch to forget. Pinned in `integration/rls/pull_world.test.ts`.
- **Removing a photo now reaches the server, and that is what makes replacing one work.** `media.detach` deletes the row and then the object — the opposite order to the avatar path, for the reason `docs/backend.md` gives. A photo taken back mid-upload cannot be recalled, so its queue entry is marked `cancelled` and the landed object is queued for deletion instead of having a row written for it; the mark rides on the entry so a force-quit cannot lose it. Driven through the real chip in `src/overlays/__tests__/taskPhoto.test.tsx`.
- **A photo removed on one device disappears on the other at the next pull, not before.** There is no push for it: the pull answering with no row for that goal *is* the signal, which is why an empty answer has to be authoritative and a silent one must never be. Up to 60s, or sooner if realtime fires for another reason — `task_media` is not in the realtime publication, so neither an attach nor the screener's `pending → ready` flip wakes anybody on its own.
- **Signed URLs live in memory for an hour and never reach the disk.** `lib/mediaUrl.ts` caches them, `write()` strips them out of persisted moments, and sign-out clears the cache beside the avatar one. Two reasons, both real: a week-long URL in AsyncStorage is a bearer token in a file, and a URL outlives the photo — `media.detach` removes the row and the object, and an already-minted link goes on resolving until it expires.
- **Photos are not rendered on the Global tab.** Bots do not attach them, and that feed has no audience behind it; rendering media there would be public image hosting by the back door.
- **Demo mode shows no photos at all.** `seedMoments` builds from fixtures and makes zero network calls, so there is nothing to sign. An empty demo feed is not this feature being broken.
- **One orphaned object is sitting in the `task-media` bucket.** 552 KB, from the slice-C verification on 2026-08-19; its row and its task were deleted but the storage object was not. It is unreachable — `private.can_see_media` answers false for it, because no `ready` row claims it and the task it names is gone — so it costs storage and nothing else. Removing it needs the service-role key or the dashboard, and there is no server-side sweep for orphans in either photo bucket.
...
- **Avatar upload, screening, and the resume path have not been run on a physical device.** Same limit as everything else in this list. The two-device run above covered sync with two live accounts, but it went nowhere near photos, and it was still simulators.
