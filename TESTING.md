...
- **A goal photo removed on one device is not removed on another until that device syncs.** Nothing pushes the removal — `pull_world` returns no media, so a second device's reducer keeps showing a photo whose row is already gone. Harmless while only the owner sees photos; it becomes a real staleness bug the moment the read path lands.
