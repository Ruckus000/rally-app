...
- **Nothing verifies the avatar screener is any good.** What's tested is the plumbing and the fail-closed verdict shape (`imageVerdict.mjs`, pinned in `src/lib/__tests__/imageVerdict.test.ts`) — not whether the model's judgement on an actual photo is right. That needs a corpus of real images and a human looking at the results, which this pass did not build.
- **The real Gemini API was never called for image screening during development.** `screen-image`'s wire shape was exercised against a stub `LLM_BASE_URL` (see `supabase/functions/README.md`) and documented-verified against Gemini's published response shape, not hit live. The first real upload against a deployed function with `GEMINI_API_KEY` set is genuinely untested.
- **Screening shares `rate-goal`'s 200/day cap** — one counter (`bump_llm_usage`), not a separate budget for photos. Over the cap, `screen-image` refuses and deletes the pending upload rather than leaving it stranded; a burst of avatar uploads can spend the same quota `rate-goal` needs for pricing goals.
...
- **`task_media` (photos on goals, on an unmerged branch) currently ships with no screening at all**, so a goal photo would go straight to `ready` while an avatar cannot. That asymmetry is worth closing before both land on `main` together.
